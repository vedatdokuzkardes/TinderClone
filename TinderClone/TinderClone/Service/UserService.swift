...
//  TinderClone
//
//  Created by Vedat Dokuzkardeş on 29.12.2023.
//
...
struct UserService{
    ...
    static func fetchSwipeCardUsers(user: User) async throws -> [User] {
        let query = Firestore.firestore().collection(COLLECTION_USER)
            .whereFilter(
                Filter.andFilter([Filter.whereField("gender", in: user.preference == TinderGender.unspecified.rawValue ?
                                                    [TinderGender.man.rawValue, TinderGender.woman.rawValue, 
                                                     TinderGender.unspecified.rawValue] : [user.preference]),
